...
//  Dicho
//
//  Created by Tyler Droll on 12/9/12.
...
- (void)viewDidLoad
{
    ...
    resultsTable.backgroundColor = [UIColor colorWithRed:0.94 green:0.98 blue:1.0 alpha:1.0];

    resultsTable.sectionFooterHeight= 0.0;
    resultsTable.sectionHeaderHeight = 8.0;
    self.navigationItem.rightBarButtonItem.enabled = NO;
    NSString *fullDichoString = [prefs objectForKey:@"mainSelectedDichoString"];
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath
{
    if(indexPath.section==0){
        ...
        float startingX = 290;
        secondAnswerBar.frame = CGRectMake(startingX-secondWidth, 30, secondWidth, 30);
...
- (void)tableView:(UITableView *)tableView didSelectRowAtIndexPath:(NSIndexPath *)indexPath
{
    if(indexPath.section==3){
        [prefs setObject:dichoID forKey:@"mainSelectedDichoID"];
        [self performSegueWithIdentifier:@"resultsToShowAnswerers" sender:self];
    }
}
...
-(void)parseGoodResults{
    ...
    [resultsTable reloadRowsAtIndexPaths:rowsToReload withRowAnimation:UITableViewRowAnimationNone];
