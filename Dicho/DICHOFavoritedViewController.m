//
//  DICHOFavoritedViewController.m
//  Dicho
//
...
#import "DICHOFavoritedViewController.h"
...
@interface DICHOFavoritedViewController ()

@end

@implementation DICHOFavoritedViewController
...
- (void)viewDidLoad
{
    ...
    favoritesTable.sectionFooterHeight= 3.0;
    favoritesTable.sectionHeaderHeight = 3.0;
    favoritesTable.backgroundView = nil;
    favoritesTable.backgroundColor = [UIColor colorWithRed:0.94 green:0.98 blue:1.0 alpha:1.0];
    ...
    NSString *strURL = [NSString stringWithFormat:@"http://dichoapp.com/files/getFavoritedDichos3.php?displayedNumber=0&userID=%@", [prefs objectForKey:@"userID"]];
...
-(void)loadUserImage:(NSNumber*)aSectionNumber{
    ...
    if(imageData != nil){
        ...
        [self.favoritesTable performSelectorOnMainThread:@selector(reloadRowsAtIndexPaths:withRowAnimation:) withObject:rowsToReload waitUntilDone:NO];
    }
}
...
-(IBAction)loadMore:(id)sender{
    if(loadedAll==NO){
        if(loadingMore == NO){
            ...
            NSString *strURL = [NSString stringWithFormat:@"http://dichoapp.com/files/getFavoritedDichos3.php?displayedNumber=%d&userID=%@",[dichoIDsArray count], [prefs objectForKey:@"userID"]];
            NSURLRequest* request = [NSURLRequest requestWithURL:[NSURL URLWithString:strURL] cachePolicy:NSURLRequestUseProtocolCachePolicy timeoutInterval:10.0];
            getDichosConnection = [[NSURLConnection alloc] initWithRequest:request delegate:self];
        }
    }
...
- (IBAction)showThePicture:(id)sender {
    NSIndexPath *indexPath = [self.favoritesTable indexPathForCell:(UITableViewCell*)[[[sender superview] superview]superview]];
...
-(void)handleGoodVote{
    ...
    [favoritesTable reloadRowsAtIndexPaths:rowsToReload withRowAnimation:UITableViewRowAnimationNone];
...
-(IBAction)goToComments:(id)sender{
    NSIndexPath *indexPath = [favoritesTable indexPathForCell:(UITableViewCell*)[[[sender superview] superview]superview]];
