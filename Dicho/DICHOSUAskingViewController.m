...
@interface DICHOSUAskingViewController () 


@end

@implementation DICHOSUAskingViewController
...
- (void)viewDidLoad
{
    ...
    NSString *strURL = [NSString stringWithFormat:@"http://dichoapp.com/files/askingUserInfos.php?asking=%@&displayedNumber=0&userID=%@", selectedUserID, userID];
    ...
    askingInfosConnection = [[NSURLConnection alloc] initWithRequest:request delegate:self];
...
- (void)tableView:(UITableView *)tableView didSelectRowAtIndexPath:(NSIndexPath *)indexPath
{
    if(indexPath.row==[userIDsAsking count]){
        ...
        NSString *strURL = [NSString stringWithFormat:@"http://dichoapp.com/files/askingUserInfos.php?asking=%@&displayedNumber=%d&userID=%@", selectedUserID, [userIDsAsking count], userID];
        NSURLRequest* request = [NSURLRequest requestWithURL:[NSURL URLWithString:strURL] cachePolicy:NSURLRequestUseProtocolCachePolicy timeoutInterval: 15.0];
        askingInfosConnection = [[NSURLConnection alloc] initWithRequest:request delegate:self];
    }
}
...
- (void)connection:(NSURLConnection *)connection didReceiveResponse:(NSURLResponse *)response {
    if(connection==askingInfosConnection){
        askingInfosData = [[NSMutableData alloc] init];
    }
}

- (void)connection:(NSURLConnection *)connection didReceiveData:(NSData *)data {
    if(connection==askingInfosConnection){
        [askingInfosData appendData:data];
    }
}
...
-(void)parseAskingInfosData{
    NSString *returnedString = [[NSString alloc] initWithData:askingInfosData encoding: NSUTF8StringEncoding];
    ...
    if([returnedString isEqualToString:@""]){
        ...
        int existingRows = [userIDsAsking count];

        NSArray *infoArray = [returnedString componentsSeparatedByString:@","];
        for(int i=0; i<infoArray.count-1; i=i+4){
            [userIDsAsking addObject:[infoArray objectAtIndex:i]];
            [namesAsking addObject:[infoArray objectAtIndex:i+1]];
            [usernamesAsking addObject:[infoArray objectAtIndex:i+2]];
        ...
        self.tableView.userInteractionEnabled = YES;
        for(int i=existingRows; i<[userIDsAsking count]; i++){
...
-(void)loadUserImage:(NSNumber*)aRowNumber{
    ...
    NSURL * imageURL = [NSURL URLWithString:[NSString stringWithFormat:@"http://dichoapp.com/userImages/%@.jpeg", [userIDsAsking objectAtIndex:rowNumber]]];
...
-(IBAction)goToUser:(id)sender{
    ...
    DICHOSingleUserViewController *nextVC = [[DICHOSingleUserViewController alloc] initWithUsername: [usernamesAsking objectAtIndex:indexPath.row] askerID:[userIDsAsking objectAtIndex:indexPath.row]];
